 ...
 * <b>CLASS</b>
 * 
 * Objeto responsavel pelas operações de registro de movimentações de ENTRADA de 
 * produtos dentro do sistema.
 * 
 * @package   App\Model\DAO
 * @author    Manoel Louro <manoel.louro@redetelecom.com.br>
 * @date      08/07/2021
 */
class AlmoxarifadoProdutoEntradaDAO extends BaseDAO {

    /**
     * Nome da tabela principal.
     * @var string
     */
    static $NOME_TABELA = 'amx_produto_entrada';
     ...
     * 
     * 
     * @param     EntidadeAlmoxarifadoProdutoEntrada $entidade Entidade carregada
     * @return    integer Resultado da operação
     * @author    Manoel Louro <manoel.louro@redetelecom.com.br>
     * @date      08/07/2021
     */
    function setRegistro(EntidadeAlmoxarifadoProdutoEntrada $entidade) {
        $retorno = 0;
        try {
            $this->insert(
                    self::$NOME_TABELA,
                ...
                ':valor_entrada' => intval($entidade->getValorEntrada()),
     ...
     * @return    EntidadeAlmoxarifadoProdutoEntrada Entidade carregada
     * @author    Manoel Louro <manoel.louro@redetelecom.com.br>
     * @date      08/07/2021
     */
        ...
        if ($registroID > 0) {
            try {
                ...
                if ($resultado && $resultado->rowCount()) {
                    ...
                    $retorno['valorAnterior'] = intval($registro['valor_anterior']);
                    $retorno['valorEntrada'] = intval($registro['valor_entrada']);
        ...
        try {
            ...
            if ($resultado && $resultado->rowCount()) {
                ...
                foreach ($registros as $value) {
                    ...
                    $registro['valorEntrada'] = intval($value['valor_entrada']);
     ...
     * @date      22/07/2021
     */
    function getListaHistoricoProduto($produtoID, $numeroPagina, $registroPorPagina) {
        $lista = [];
        //DETERMINA A CONSULTA
        $numeroPagina = $numeroPagina - 1;
        $numeroPagina = $numeroPagina < 1 ? $numeroPagina = 0 : ($numeroPagina * $registroPorPagina);
        if ($produtoID > 0) {
            try {
                $resultado = $this->select(
                        ...
                        INNER JOIN " . AlmoxarifadoProdutoDAO::$NOME_TABELA . " AS p ON e.fk_produto = p.id 
                        INNER JOIN " . EmpresaDAO::$NOME_TABELA . " AS emp ON p.fk_empresa = emp.id 
                        WHERE e.fk_produto = " . $produtoID . " ORDER BY e.id DESC LIMIT " . $numeroPagina . ", " . $registroPorPagina
                );
                if ($resultado && $resultado->rowCount()) {
                    $registros = $resultado->fetchAll();
                    foreach ($registros as $value) {
     ...
     * paginação do sistema.
     * 
     * @param     string $produtoID Produto informado
     * @return    integer Total de registros
     * @author    Manoel Louro <manoel.louro@redetelecom.com.br>
     * @date      23/07/2021
     */
    function getListaHistoricoProdutoTotal($produtoID) {
        $quantidade = 0;
        if ($produtoID > 0) {
            try {
                $resultado = $this->select(
    ...
    private function setCarregarEntidade($registro) {
        ...
        if (!empty($registro)) {
            ...
            $entidade->setValorAnterior(intval($registro['valor_anterior']));
            $entidade->setValorEntrada(intval($registro['valor_entrada']));
